...
//Global Variable
var tenDayPop;
async function getForecasted10DayPoP() {
  // db.WeatherFC.find({},{_id:0, pop:1}, function (err, tenDayPop) {
    await db.WeatherFC.find({},{_id:0, pop:1}, function (err, result) {
                                        if (err) {
...
async function getCityWateringRestrictions() {
  // db.CityWateringRest.find({}, function (err, cityWateringRestrictions) {
    await db.CityWateringRest.find({}, function (err, result) {
                                   if (err) {
                                     console.log ("Find Error", err);
                                   }
                                   else {
...
const computeNextWaterDateTime = async (zoneNumber,inMemZone) => {
  ...
  if (inMemZone[0].watsonImageClass === 'Healthy') {
      ...
      await db.WateringZone.update({"zoneNumber": zoneNumber}, {$set: {"zoneStatus": "AW", "projectedNextWaterDateTime": nextWaterDateTimeEpoch}}, function(err, raw){
         ...
         if (curDay < wDay1index) { //straightforward. The next watering day is the same as wDay1
          ...
          nextWaterDateTimeEpoch = todayAtZeroTimeEpoch + 24*60*60*(wDay1index-curDay);
            ...
            if (curDay > wDay1index) {
              ...
              nextWaterDateTimeEpoch = todayAtZeroTimeEpoch + 24*60*60*(wDay1index+7-curDay);
            }
          ...
          inMemZone.projectedNextWaterDateTime = nextWaterDateTimeEpoch;
          await db.WateringZone.update({"zoneNumber": zoneNumber}, {$set: {"zoneStatus": "NW", "projectedNextWaterDateTime": nextWaterDateTimeEpoch}}, function(err, raw){
            if (err) { console.log("database call to set zoneStatus failed with error:", err);
            }
            else {console.log ("database call to set zoneStatus, for zone: ", zoneNumber, " and projectedNextWaterDateTime succeeded with raw = ", raw)}
          });
...
async function updateWateringZoneDB(zoneNumber){
  await db.WateringZone
...
module.exports = {
  generateZoneRecos: async function(req, res) {
    console.log("In controller/generateRecoController - READY TO DO GREAT THINGS");
    await getForecasted10DayPoP();
    await getCityWateringRestrictions();
    for (let index = 1; index < 7; index++) {
      await updateWateringZoneDB(index);
    }
